...
const Keypad = ({ keyHistory, onClick }: IKeypad) => {
  return (
    <div className="flex flex-col justify-center items-center h-40 sm:h-56 max-h-full max-w-full w-[660px] gap-y-1">
      {Object.entries(keys).map(([row, vals]) => (
        <div className="flex w-full h-full gap-x-1" key={row}>
          <EmptyKey row={row} key={`first-${row}`} />
          {vals.map(({ key }) => (
            <Key key={key} char={key} onClick={onClick} state={keyHistory[key]} />
          ))}
